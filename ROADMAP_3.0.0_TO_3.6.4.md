...
- 3.4.11 到 3.4.20 固定为分流页产品化补课路线。3.4.10 只能代表 IPv6/DNS 地基完成，不能代表分流页成熟。
- 3.5.x 在 3.4.20 通过前暂停推进，不能用后续故障分类路线掩盖分流页仍是半成品的问题。
...
| 3.4.11 | 分流页真实完成度审计 | 输出分流页缺口清单，区分工程完成、产品可用、成熟可交付，不能只用脚本门禁代替体验验收。 |
| 3.4.12 | 分流页信息架构重排 | 页面按“当前如何走、我要改什么、改动草稿、规则明细、风险诊断”组织，去掉调试页式堆叠。 |
| 3.4.13 | 普通用户分流场景向导 | 网站、应用、地区、流媒体、游戏平台、AI 服务等场景用自然语言选择，不要求用户理解 YAML。 |
| 3.4.14 | 草稿中心产品化 | 草稿必须显示来源、意图、影响范围、冲突、验证结果和下一步动作，不能只显示规则文本。 |
| 3.4.15 | 分流应用预检闭环 | 应用前完成配置生成、目标存在、顺序冲突、回滚点、核心热重载预检，不改变当前连接。 |
| 3.4.16 | 分流安全应用与回滚 | 用户确认后才写配置；写入必须原子化、可回滚、失败不污染当前可用配置。 |
| 3.4.17 | 分流生效验证 | 应用后可验证指定网站/应用实际走向，结果用“已走代理/已直连/未命中/被上层规则覆盖”表达。 |
| 3.4.18 | 分流诊断与错误解释 | 冲突、未命中、目标缺失、自动策略组覆盖、规则顺序错误都给出普通用户能执行的建议。 |
| 3.4.19 | 分流页交互与性能验收 | 快速切页、输入、生成草稿、验证、应用、滚动规则列表都不能卡 UI；不同窗口尺寸不乱。 |
| 3.4.20 | 分流页产品化验收 | 普通用户能独立完成分流规则创建、验证、应用、撤销；安全、性能、回滚和安装包候选门禁通过。 |
