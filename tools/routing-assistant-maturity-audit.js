...
check('3.3.8 simple and advanced surfaces are separated', appJs.includes('routingAssistantView') && appJs.includes('routingSimpleViewBtn') && appJs.includes('routingAdvancedViewBtn') && stylesCss.includes('.routing-assistant[data-view="advanced"]'), 'simple/advanced view');
...
check('assistant layout has stable draft rows', stylesCss.includes('.routing-draft-row') && stylesCss.includes('grid-template-columns: minmax(180px, 1fr) 70px 64px 64px') && stylesCss.includes('overflow: auto'), 'stable layout');
