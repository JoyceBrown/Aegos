...
const manualNodeRuntimeRs = read('src-tauri/src/manual_node_runtime.rs');
...
check(
  'fixed-node settings use the same candidate transaction',
  mainRs.includes('fn stage_settings_deployment(')
    && manualNodeRuntimeRs.includes('"Fixed node save"')
    && manualNodeRuntimeRs.includes('"Fixed node delete"')
    && manualNodeRuntimeRs.includes('self.stage_settings_deployment("Fixed node save")')
    && manualNodeRuntimeRs.includes('self.stage_settings_deployment("Fixed node delete")')
    && (manualNodeRuntimeRs.match(/\.rollback_with_runtime\(/g) || []).length >= 2
    && (manualNodeRuntimeRs.match(/\.complete_verified\(/g) || []).length >= 2,
  'main.rs stages settings; manual_node_runtime.rs owns fixed-node apply and rollback'
);
