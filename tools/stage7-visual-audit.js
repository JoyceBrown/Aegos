...
check('third-party icon license and pinned source are preserved', fluentLicense.includes('MIT License') && licenseAudit.includes('9a1129bb2432b163b48044341664c68a3c100908') && licenseAudit.includes('Microsoft Fluent UI System Icons'));
check('legacy hand-drawn icons are removed except for the Aegos brand mark', (css.match(/data:image\/svg\+xml;base64/g) || []).length === 38 && (css.match(/data:image\/svg\+xml,%3Csvg/g) || []).length === 1, `${(css.match(/data:image\/svg\+xml;base64/g) || []).length} Fluent / ${(css.match(/data:image\/svg\+xml,%3Csvg/g) || []).length} brand`);
