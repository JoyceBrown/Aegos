...
check(
  '3.6.24 Tauri command snapshot remains preserved as historical evidence',
  baseline.version === '3.6.24' && Object.keys(baseline.literalInvokeCounts || {}).length > 0
);
...
check('root ownership is consolidated', (css.match(/^:root\s*\{/gm) || []).length <= 3, `${(css.match(/^:root\s*\{/gm) || []).length} root blocks`);
...
check(
  'button hover translation is limited to one pixel',
  css.includes('transform: translateY(-1px)') &&
    !/button[^{]*:hover[^{]*\{[^}]*translateY\(-([2-9]|\d{2,})px\)/gs.test(css)
);
...
check('node icon actions have Chinese accessible names', ['测试节点延迟', '编辑节点', '添加网站或应用分流规则', '收藏节点'].every((label) => app.includes(label)) && !app.includes("ariaLabel: 'test delay'") && !app.includes('ariaLabel: `select ${name}`'));
check('home favorites use the centralized icon registry', app.includes("icon(`star ${favorite ? 'icon-star-filled' : 'icon-star'}`)"));
check(
  'Fluent icon subset is local, embedded, complete, and runtime-free',
  fluentSvgFiles.length === 38 &&
    (css.match(/data:image\/svg\+xml;base64/g) || []).length === 38 &&
    !html.includes('fluentui-system-icons') &&
    !app.includes('fluentui-system-icons'),
  `${fluentSvgFiles.length} local assets / ${(css.match(/data:image\/svg\+xml;base64/g) || []).length} embedded icons`
);
...
check('stage 7 important patches remain bounded', (css.match(/!important/g) || []).length <= 13, `${(css.match(/!important/g) || []).length} !important declarations`);
