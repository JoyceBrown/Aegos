...
try {
  ...
  const report = await evaluate(page, `(async () => {
    ...
    const navPages = ['home', 'nodes', 'connections', 'routing', 'profiles', 'diagnostics', 'logs', 'settings'];
    const navDurations = [];
  ...
  if (report.calls.routingAfterSettle < report.calls.routingBeforeQuiet + 1) failures.push('settled routing page did not refresh after quiet period');
  ...
  const result = { ok: failures.length === 0, failures, longTaskBudget: { severeMs: 180, maxMs: 300, maxSevereCount: 1 }, ...report };
