...
check('verified takeover completion cannot leave Windows state half-committed', takeover.includes('pub fn complete_verified') && takeover.includes('let rollback_result = rollback();') && takeover.includes('Completion journal failed and the previous Windows network state was restored') && main.match(/transaction\.complete_verified\(/g)?.length >= 5 && main.includes('fn restore_settings_snapshot'));
