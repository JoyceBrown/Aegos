...
check('speed tests do not mark the whole foreground busy', appJs.includes('speedTestStarting') && appJs.includes('function testNodes(button = null)') && appJs.includes("$('#quickTestBtn').onclick = (event) => testNodes(event.currentTarget)") && !appJs.includes("$('#quickTestBtn').onclick = (event) => runButtonAction"), 'non-blocking speed test start');
