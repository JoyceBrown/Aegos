...
check('smart-mode outbound IP lookup routes through hidden current-node group', mainRs.includes('OUTBOUND_IP_RULE_DOMAINS') && mainRs.includes('AEGOS_OUTBOUND_IP_GROUP') && mainRs.includes('fn upsert_outbound_ip_group') && mainRs.includes('fn sync_outbound_ip_group_selection') && mainRs.includes('fn insert_outbound_ip_rules') && mainRs.includes('DOMAIN,{domain},{target}') && mainRs.includes('outbound_ip_lookup_rules_use_internal_current_node_group'), 'smart-mode outbound IP internal routing');
...
check('speed tests stream completed nodes quickly', mainRs.includes('mpsc::channel') && mainRs.includes('speed_test_ordered_targets') && mainRs.includes('emit_speed_test_event') && mainRs.includes('Arc::new(client)') && mainRs.includes('speed.recommended = speed_recommendation') && appJs.includes("listen('aegos-speed-test', handleSpeedTestEvent)"), 'per-node Tauri events with a polling watchdog');
...
check('speed tests use protocol-aware adaptive scheduling', mainRs.includes('fn protocol_concurrency') && mainRs.includes('fn speed_test_ordered_targets') && mainRs.includes('fn adaptive_speed_concurrency') && mainRs.includes('protocol_primary_timeout_ms') && mainRs.includes('"tuic" => 6') && mainRs.includes('collect_proxy_targets'), 'protocol-aware speed path');
...
check('subscription source preflight is wired', subscriptionRuntimeRs.includes('pub(crate) struct ProfileSourceSummary') && subscriptionRuntimeRs.includes('pub(crate) struct ProfileSource') && subscriptionRuntimeRs.includes('pub(crate) fn summarize_source(') && mainRs.includes('mod subscription_runtime') && mainRs.includes('fn summarize_profile_source') && mainRs.includes('subscription_runtime::summarize_source(config, format, unsupported_lines)') && mainRs.includes('subscription download returned empty content') && mainRs.includes('download_profile_source_url_diagnostic') && mainRs.includes('config_pipeline::preflight_profile_source(source.config, &profile, &settings)') && mainRs.includes('let patched = runtime.config') && mainRs.includes('"runtime-preflight"') && appJs.includes('profile-source-summary'), 'subscription preflight');
check('subscription import failures have actionable diagnostics', mainRs.includes('fn subscription_diagnostic') && mainRs.includes('parse_uri_subscription_source_diagnostic') && mainRs.includes('"unsupported-format"') && mainRs.includes('"unsupported-protocol"') && mainRs.includes('"yaml-parse"') && mainRs.includes('subscription_diagnostics_classify_unsupported_protocols') && pkg.scripts?.['audit:subscription'] === 'node tools/subscription-diagnostics-audit.js', 'subscription diagnostics');
check('protocol capability matrix is explicit', mainRs.includes('const AEGOS_URI_PROTOCOLS') && !mainRs.includes('const MIHOMO_PROXY_TYPES') && coreRuntimeRs.includes('pub const SUPPORTED_PROXY_TYPES') && coreRuntimeRs.includes('pub fn supports_proxy_type') && coreRuntimeRs.includes('pub fn protocol_capabilities_json') && mainRs.includes('core_runtime::protocol_capability_summary') && coreRuntimeRs.includes('protocolCapabilities') && coreRuntimeRs.includes('Config preflight failed: unsupported proxy type(s)') && mainRs.includes('preflight_rejects_core_unsupported_proxy_type') && mainRs.includes('manual_hy2_node_is_normalized_to_hysteria2') && coreRuntimeRs.includes('runtime_protocol_capabilities_normalize_and_report_current_contract'), 'protocol capability matrix');
...
check('node switch preflight is wired', mainRs.includes('fn validate_proxy_selection_from_groups') && mainRs.includes('Node switch preflight failed') && mainRs.includes('Node switch preflight passed') && mainRs.includes('node_switch_preflight_validates_group_and_proxy') && mainRs.includes('validate_proxy_selection_from_groups(&groups, group, proxy)?'), 'node switch preflight');
...
check('connection failure classifier is wired', coreRuntimeRs.includes('pub fn classify_failure_reason') && coreRuntimeRs.includes('pub fn classified_error') && coreRuntimeRs.includes('runtime_failure_reason_classifier_covers_common_connection_failures') && mainRs.includes('return Err(core_runtime::classified_error("Node switch", err));') && mainRs.includes('core_runtime::classify_failure_reason(&entry.line)') && !mainRs.includes('fn classify_failure_reason') && !mainRs.includes('fn classified_error'), 'connection failure classifier');
...
check('speed testing does not block sidebar page switching', appJs.includes('function testNodes(button = null)') && !appJs.includes("$('#quickTestBtn').onclick = (event) => runButtonAction") && interactionSmoke.includes('speed test blocked sidebar page switching'), 'speed test navigation remains responsive');
...
check('large node lists are windowed, debounced, and row-cached', appJs.includes('nodeInitialRenderLimit = 36') && appJs.includes('nodeRenderLimit = 96') && appJs.includes('interactiveNodeRenderLimit') && appJs.includes('interactiveNodeCandidateLimit') && appJs.includes('homeNodeRenderLimit = 8') && appJs.includes('rowRenderSettleMs = 320') && appJs.includes('nodeCandidateLimit') && appJs.includes('nodeRows.length < nodeCandidateLimit') && appJs.includes('const interactiveRender = largeList && (isForegroundHot() || isSpeedTestActive())') && appJs.includes('const nodeVisibleLimit = largeList ? visibleNodeLimit : Math.max(nodeInitialRenderLimit, nodeRows.length)') && appJs.includes('sortNodeRows(nodeRows).slice(0, nodeVisibleLimit)') && appJs.includes('matchingNodeCount') && appJs.includes('nodeRowStaticCache') && appJs.includes('function normalizeNodeItemCached'), 'windowed/cached node rendering');
...
check('traffic stream uses CoreController runtime status snapshot reader', !mainRs.includes('fn traffic_snapshot(&self)') && appStatusBody.includes('.status_traffic_snapshot_or_idle(observed_running, &previous_traffic)') && coreRuntimeRs.includes('pub fn traffic_snapshot(&self, timeout_ms: u64)') && coreRuntimeRs.includes('pub fn status_traffic_snapshot(&self)') && coreRuntimeRs.includes('pub fn status_traffic_snapshot_or_idle(') && coreRuntimeRs.includes('pub fn idle_traffic_snapshot()') && coreRuntimeRs.includes('pub const STATUS_TRAFFIC_TIMEOUT_MS') && !mainRs.includes('controller("GET", "/traffic"'), '/traffic snapshot');
check('status heartbeat is lightweight', statusObservationBody.includes('self.core_controller()') && statusAssemblyBody.includes('core_runtime::status_surface_json') && statusAssemblyBody.includes('recent_logs(120)') && appStatusBody.includes('core.status_observation()') && appStatusBody.includes('refresh_lan_ip.then(primary_lan_ip)') && appStatusBody.includes('core.status_from_observed_traffic(') && coreRuntimeRs.includes('pub fn status_surface_json(') && coreRuntimeRs.includes('pub fn runtime_status_json(') && coreRuntimeRs.includes('"version": JsonValue::Null') && !appStatusBody.includes('"/version"') && !appStatusBody.includes('traffic_snapshot(120)'), 'app_status lock-split snapshot');
