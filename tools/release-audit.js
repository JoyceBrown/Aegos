...
check('speed tests stream completed nodes quickly', mainRs.includes('mpsc::channel') && mainRs.includes('normal_targets') && mainRs.includes('Arc::new(client)') && appJs.includes('const speedTestPollMs = 300'), 'incremental speed test results');
check('TUIC speed tests use fast low-concurrency path', mainRs.includes('is_tuic_protocol') && mainRs.includes('2800') && mainRs.includes('(tuic_targets, 12usize)') && mainRs.includes('collect_proxy_targets'), 'tuic fast path');
...
check('page switching uses cached absolute layers', pageStyleBlock.includes('position: absolute') && pageStyleBlock.includes('display: grid') && pageStyleBlock.includes('content-visibility: hidden') && stylesCss.includes('content-visibility: visible') && appJs.includes('pageNavSettleMs') && appJs.includes('pageCacheState') && !pageStyleBlock.includes('display: none'), 'cached page layers');
check('large node lists are windowed and debounced', appJs.includes('nodeRenderLimit = 60') && appJs.includes('homeNodeRenderLimit = 8') && appJs.includes('rowRenderSettleMs = 320') && appJs.includes('visibleNodeRows'), 'windowed node rendering');
...
check('status heartbeat is lightweight', statusBody.includes('traffic_snapshot(120)') && statusBody.includes('"version": JsonValue::Null') && !statusBody.includes('"/version"'), 'app_status snapshot');
