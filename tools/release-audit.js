...
check('home node mode filters are present', ['region', 'favorite', 'frequent', 'fixed'].every((mode) => indexHtml.includes(`data-home-mode="${mode}"`)) && indexHtml.indexOf('data-home-mode="region"') < indexHtml.indexOf('data-home-mode="frequent"') && !indexHtml.includes('data-page-jump="nodes"') && appJs.includes("homeNodeMode = 'region'") && appJs.includes("homeNodeMode: 'region'") && appJs.includes('compareHomeRows') && interactionSmoke.includes('home did not default to common regions') && interactionSmoke.includes('all nodes shortcut still renders on home'), 'home node modes');
