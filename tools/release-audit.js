...
check('window remains resizable without edge drag overlay', tauri.app?.windows?.[0]?.resizable === true && !indexHtml.includes('window-edge-frame') && !stylesCss.includes('.window-drag-edge'), `resizable=${tauri.app?.windows?.[0]?.resizable}`);
...
check('quick actions have one-line subscription and protection controls', (indexHtml.match(/id="quick[A-Za-z]+Btn"/g) || []).length === 6 && indexHtml.includes('id="quickProfileBtn"') && indexHtml.includes('id="quickKillBtn"') && indexHtml.includes('id="profileMenu"') && !indexHtml.includes('id="quickIpBtn"') && !indexHtml.includes('id="quickTunBtn"') && !indexHtml.includes('id="quickCopyProxyBtn"') && !indexHtml.includes('id="smartRecoverBtn"') && !indexHtml.includes('id="quickModeBtn"') && appJs.includes('function renderQuickProfileMenu') && appJs.includes('function toggleProfileMenu') && appJs.includes("updateSetting('killSwitchEnabled'") && interactionSmoke.includes('manual outbound IP quick action still renders') && interactionSmoke.includes('quick subscription menu did not open'), 'one-line quick subscription/protection menu');
...
check('speed test does not switch proxies or auto-start core', testNodesBody.includes("invoke('start_proxy_delay_test'") && !testNodesBody.includes("runBackgroundJob('changeProxy'") && !testNodesBody.includes('selectBestProxyJob') && speedTestBody.includes('Speed test skipped because core controller is unavailable') && !speedTestBody.includes('restart_core_preserving_proxy') && interactionSmoke.includes('speed test triggered a proxy switch') && interactionSmoke.includes('batch speed test triggered a proxy switch'), 'test only updates delay/recommendation');
