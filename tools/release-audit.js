...
check('quick actions have compact subscription and protection controls', (indexHtml.match(/id="quick[A-Za-z]+Btn"/g) || []).length >= 7 && indexHtml.includes('id="quickProfileBtn"') && indexHtml.includes('id="quickKillBtn"') && indexHtml.includes('id="profileMenu"') && !indexHtml.includes('id="quickIpBtn"') && !indexHtml.includes('id="quickTunBtn"') && !indexHtml.includes('id="quickCopyProxyBtn"') && appJs.includes('function renderQuickProfileMenu') && appJs.includes('function toggleProfileMenu') && appJs.includes("updateSetting('killSwitchEnabled'") && interactionSmoke.includes('manual outbound IP quick action still renders') && interactionSmoke.includes('quick subscription menu did not open'), 'quick subscription/protection menu');
...
check('disconnect protection verifies firewall state', mainRs.includes("DefaultOutboundAction -ne 'Block'") && mainRs.includes('$rules.Count -lt 1') && mainRs.includes('$rules.Count -gt 0') && mainRs.includes('throw $failure'), 'disconnect protection firewall verification');
check('speed test does not switch proxies', testNodesBody.includes("invoke('start_proxy_delay_test'") && !testNodesBody.includes("runBackgroundJob('changeProxy'") && !testNodesBody.includes('selectBestProxyJob') && interactionSmoke.includes('speed test triggered a proxy switch') && interactionSmoke.includes('batch speed test triggered a proxy switch'), 'test only updates delay/recommendation');
