...
check('manual fixed nodes are editable and persisted', mainRs.includes('manual_nodes') && mainRs.includes('fn save_manual_node') && configPipelineRs.includes('fn apply_manual_nodes') && configPipelineRs.includes('insert_manual_node_into_config') && indexHtml.includes('id="nodeEditorOverlay"') && indexHtml.includes('id="addFixedNodeBtn"') && indexHtml.includes('id="nodeEditUsernameInput"') && appJs.includes("invoke('save_manual_node'") && appJs.includes('payload.username = username') && appJs.includes('collectNodeEditorPayload') && interactionSmoke.includes('authenticated SOCKS5 fixed node did not save its username and password'), 'manual fixed nodes with HTTP/SOCKS5 authentication');
