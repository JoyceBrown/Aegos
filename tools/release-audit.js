...
check('home separates current and recommended nodes', indexHtml.includes('id="currentNodeName"') && indexHtml.includes('id="recommendedNodeName"') && indexHtml.includes('id="switchRecommendedBtn"') && appJs.includes('function renderHomeNodeSummary') && appJs.includes("setBestButton.textContent = '\\u5207\\u6362\\u5230\\u63a8\\u8350'"), 'current/recommended home summary');
...
check('recommended chips do not connect directly', appJs.includes('推荐节点：') && appJs.includes('点击“切换到推荐”才会连接') && interactionSmoke.includes('recommended node chip switched proxy directly'), 'recommended chip is informational');
