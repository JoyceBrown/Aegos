...
check('system proxy incomplete state is visually flagged in status center', appJs.includes("$('#proxyState').classList.toggle('is-danger', !systemProxyApplied)") && stylesCss.includes('.status-card dd.is-danger'), 'status-center danger state');
check('outbound IP lookup coalesces pending work and rejects stale results', appJs.includes('const seq = ++outboundIpRequestSeq') && appJs.includes('outboundIpPendingSeq = seq') && appJs.includes('if (seq !== outboundIpRequestSeq) return') && appJs.includes('outboundIpPendingSeq = 0') && appJs.includes('renderOutboundIpFromStatus(previous'), 'outbound IP sequence/pending guard');
