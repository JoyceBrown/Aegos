...
check('release version has moved to post-freeze cleanup or guarded 3.x line', pkg.version === tauri.version && (/^2\.9\.(?:29|[3-9]\d+)$/.test(pkg.version) || /^3\.(?:0\.0|1\.[0-5])$/.test(pkg.version)), `${pkg.version}/${tauri.version}`);
...
check('PowerShell runs through single hidden launcher with escaping helpers', (mainRs.match(/Command::new\("powershell\.exe"\)/g) || []).length === 1 && mainRs.includes('fn run_powershell') && mainRs.includes('CREATE_NO_WINDOW') && mainRs.includes('fn ps_escape'), 'PowerShell launcher');
