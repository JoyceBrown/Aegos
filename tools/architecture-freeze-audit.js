...
check('disconnect protection firewall is wrapped and verifiable', mainRs.includes('Invoke-AegosNetsh') && mainRs.includes('Disconnect protection enable failed') && mainRs.includes('CoreFirewallPolicyPlan::disconnect_protection()') && mainRs.includes('CoreFirewallPolicyPlan::speed_test()') && coreRuntimeRs.includes('pub struct CoreFirewallPolicyPlan') && coreRuntimeRs.includes('fn firewall_policy_contract_is_owned_by_runtime_boundary') && !mainRs.includes('fn build_speed_test_firewall_script') && !mainRs.includes('cleanup_speed_firewall'), 'firewall wrapper');
...
check('outbound IP lookup has current-node smart-mode routing', ['AEGOS_OUTBOUND_IP_GROUP', 'OUTBOUND_IP_RULE_DOMAINS', 'sync_outbound_ip_group_selection'].every((token) => mainRs.includes(token)) && configPipelineRs.includes('fn upsert_outbound_ip_group') && configPipelineRs.includes('fn insert_outbound_ip_rules'), 'outbound IP group');

check('runtime config chain uses preflight, hot reload, digest, rollback', configPipelineRs.includes('pub(crate) fn patch_config') && configPipelineRs.includes('core_runtime::preflight_runtime_config(') && !mainRs.includes('fn patch_config_with_settings') && ['hot_reload_profile', 'runtime_config_digest', 'Profile hot reload failed; falling back to restart', 'Profile switch failed and rolled back'].every((token) => mainRs.includes(token)), 'profile config chain');
...
check('frontend renders dynamic values through safe DOM nodes without legacy HTML helpers', !appJs.includes('function escapeHtml') && !/(innerHTML|outerHTML|insertAdjacentHTML|document\.write|new Function|eval\()/m.test(appJs), 'safe DOM sinks and removed dead helper');
...
check('routing keeps subscription and system rules read-only while Aegos user rules stay controlled', (!indexHtml.includes('data-page="routing"') || (indexHtml.includes('订阅/系统规则只读') && pkg.scripts?.['audit:routing-readonly'] === 'node tools/routing-readonly-audit.js' && pkg.scripts?.['audit:routing-product'] === 'node tools/routing-product-audit.js' && releaseAudit.includes('controlled routing boundary audit script exists'))) && !indexHtml.includes('data-page="rules"') && !appJs.includes('customRule') && !mainRs.includes('save_routing_rule') && mainRs.includes('fn apply_routing_drafts') && mainRs.includes('fn mark_system_routing_rules'), 'controlled routing boundary');
