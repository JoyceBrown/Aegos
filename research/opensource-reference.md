...
| 3.2.0 | 分流规则基础模型与预检 | mihomo rules / MetaCubeXD | 规则解析、目标校验、冲突诊断、回滚契约通过 |
| 3.3.0 | 分流助手入口与只读门禁 | Hiddify / mihomo rules | 普通用户无需 YAML 生成网站、应用、连接草稿 |
| 3.4.0 | IPv6 自动检测设计与实现 | sing-box/mihomo 思路 | 自动检测、防泄漏、回退提示 |
| 3.5.0 | 故障分类与智能恢复 | FlClash / mihomo diagnostics | 失败原因可见，恢复动作可取消且不侵入 |
| 3.6.0 | 节点个性化与手动节点成熟化 | v2rayN / NekoBox | 固定节点字段、导入、编辑、测速、连接、导出可验收 |
